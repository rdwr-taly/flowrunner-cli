...
def _shutdown_and_exit() -> None:
    """Stop the application and terminate the process without raising SystemExit."""
    try:
        if state["app_status"] == "running":
            _stop()
    except Exception:
        log.exception("Shutdown stop sequence failed")
    finally:
        # Small delay to allow log flushes before hard exit.
        time.sleep(0.2)
        os._exit(0)


def _handle_exit_signal(_s: int, _f: FrameType | None) -> None:
    if _shutdown_requested.is_set():
        return
    _shutdown_requested.set()
    log.info("Exit signal received, initiating graceful shutdown.")
    _thread(_shutdown_and_exit)
